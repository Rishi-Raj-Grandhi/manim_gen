...
async def generate_video(request: VideoGenerationRequest):
    ...
    try:
        ...
        # ✅ Debug env + attempt S3 upload
        print(
            "ENV CHECK:",
            "REGION=", os.getenv("AWS_REGION"),
            "BUCKET=", os.getenv("S3_BUCKET"),
            "KEY_SET=", bool(os.getenv("AWS_ACCESS_KEY_ID")),
        ...
        try:
            ...
            s3_url = video_url   # fallback to local media URL
            storage = "local"
        ...
        return {
            ...
            "video_url": s3_url,   # S3 URL (or local fallback)
            "s3_key": s3_key,      # <-- added per Step 1A
            "storage": storage,
            "result": result
        }
...
def get_presigned_url(
    key: str = Query(..., description="S3 object key, e.g. generated/video_xxx.mp4"),
    ttl: int = 3600
):
    try:
        url = presign_key(key, ttl)
        return {"url": url, "key": key, "ttl": ttl}
    except Exception as e:
